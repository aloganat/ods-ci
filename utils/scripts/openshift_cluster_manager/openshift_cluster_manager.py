...
class OpenshiftClusterManager():
    ...
    def ocm_describe(self, filter=""):
        ...
        print (ret)
        if ret is None:
            print ("ocm describe for cluster {} failed".format(self.cluster_name))
            return None
        return ret
    ...
    def is_osd_cluster_exists(self):
        ...
        if ret is None:
            print ("ocm cluster with name {} not exists!".format(self.cluster_name))
            return False
    ...
    def osd_cluster_create(self):
        ...
        cmd = "ocm create cluster --aws-account-id {} --aws-access-key-id {} --aws-secret-access-key {} --ccs --region {} --compute-nodes {} --compute-machine-type {} {}".format(self.aws_account_id, self.aws_access_key_id, self.aws_secret_access_key, self.aws_region, self.num_compute_nodes, self.aws_instance_type, self.cluster_name)
        print (cmd)
        ret = execute_command(cmd)
        print (ret)
        if ret is None:
    ...
    def get_osd_cluster_id(self):
        ...
        if cluster_name is None:
            print ("Unable to retrieve cluster ID for cluster name {}. EXITING".format(self.cluster_name))
            sys.exit(1)
    ...
    def get_osd_cluster_console_url(self):
        cluster_console_url = self.ocm_describe(filter="--json | jq -r '.console.url'")
        if cluster_console_url is None:
            print ("Unable to retrieve cluster console url for cluster name {}. EXITING".format(self.cluster_name))
            sys.exit(1)
    ...
    def get_osd_cluster_info(self, config_file = "osd_config_file.yaml"):
        ...
        # TODO: Avoid this hard coding and call create identity provider method once its ready
    ...
    def wait_for_osd_cluster_to_be_ready(self, timeout=7200):
        ...
        while(count <= timeout):
            ...
            if cluster_state == "ready":
                ...
                print ("{} is in error state. Hence exiting!!".format(self.cluster_name))
                sys.exit(1)

            time.sleep(60)
            count +=60
        if not check_flag:
            print ("{} not in ready state even after 2 hours. EXITING".format(self.cluster_name))
            sys.exit(1)
    ...
    def is_addon_installed(self, addon_name="managed-odh"):
        ...
        if addon_state == "not installed":
            print ("Addon {} not installed in cluster {}".format(addon_name, self.cluster_name))
            return False
        print ("Addon {} is installed in cluster {}".format(addon_name, self.cluster_name))
        return True
    ...
    def get_addon_state(self, addon_name="managed-odh"):
        ...
        ret = execute_command(cmd)
        if ret is None:
            print ("Failed to get {} addon state for cluster {}".format(addon_name, self.cluster_name))
            return None
    ...
    def wait_for_addon_installation_to_complete(self, addon_name="managed-odh", timeout=3600):
        ...
        while(count <= timeout):
            ...
            count +=60
        if not check_flag:
            print ("addon {} not in installed state even after 30minutes. EXITING".format(addon_name))
            sys.exit(1)
    ...
    def install_rhods(self):
        ...
        cmd = "ocm post /api/clusters_mgmt/v1/clusters/{}/addons --body={}".format(cluster_id, output_file)
        print (cmd)
        ret = execute_command(cmd)
        print (ret)
        if ret is None:
            print("Failed to install rhods addon on cluster {}".format(self.cluster_name))
            sys.exit(1)

    def create_idp(self):

        if self.idp_type == "htpasswd":
            cmd = "ocm create idp -c {} -t {} -n {} --username {} --password {}".format(self.cluster_name, self.idp_type, self.idp_name, self.htpasswd_cluster_admin, self.htpasswd_cluster_password)
        print (cmd)
        ret = execute_command(cmd)
        print (ret)
        if ret is None:
            print("Failed to add identity provider of type {}".format(self.idp_type))
            sys.exit(1)

    def add_user_to_group(self, group="cluster-admins"):

        cmd = "ocm create user {} --cluster {} --group={}".format(self.htpasswd_cluster_admin, self.cluster_name, group)
        print (cmd)
        ret = execute_command(cmd)
        print (ret)
        if ret is None:
            print("Failed to add user {} to group {}".format(self.htpasswd_cluster_admin, group))
            sys.exit(1)
...
def parse_args():
    ...
    parser.add_argument("-o", "--ocmclibinaryurl",
                        ...
                        default="https://github.com/openshift-online/ocm-cli/releases/download/v0.1.55/ocm-linux-amd64")
